...
X<-read.table("X_train.txt"); y<-read.table("y_train.txt"); st<-read.table("subject_train.txt")

# cbind all three into one big data frame (562nd column is y which is important, 563rd is st)
traindata<-cbind(y,st); traindata<-cbind(X,traindata)
...
# Change working directory back to UCI HAR Dataset so as to download features file
...
df2<-as.data.frame(colMeans(datause)); df2<-t(df2); df2<-as.data.frame(df2);
View(df2)
